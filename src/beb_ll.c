...
beb_ll_error_t beb_ll_encrypt_aes_gcm_256_v1_with_nonce(
    ...
    if (derivation_paths_count > 0) {
        ...
        for (size_t i = 0; i < derivation_paths_count; i++) {
            ...
            if (!found) {
                /* Copy path */
                filtered_paths[filtered_paths_count].count = derivation_paths[i]
                                                                 .count;
...
beb_ll_error_t beb_ll_decrypt_aes_gcm_256_v1(
    ...
    for (size_t i = 0; i < individual_secrets_count; i++) {
        ...
        beb_ll_error_t err = beb_ll_try_decrypt_aes_gcm_256(
            cyphertext, cyphertext_len, secret, nonce, &plaintext,
            &plaintext_len);
        if (err == BEB_LL_ERROR_OK && plaintext) {
            /* Parse content metadata */
            size_t offset = 0;
            err = beb_ll_parse_content_metadata(plaintext, plaintext_len,
                                                &offset, &result_out->content);
            if (err != BEB_LL_ERROR_OK) {
                free(plaintext);
                continue;
            }
