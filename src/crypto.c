...
/* Note: For Bitcoin Core integration, replace OpenSSL SHA256 with Bitcoin Core's crypto/sha256.h */

/* SHA256 using OpenSSL (to be replaced with Bitcoin Core's SHA256) */
static void sha256_hash(const uint8_t *data, size_t data_len, uint8_t hash_out[32]) {
    SHA256_CTX ctx;
    SHA256_Init(&ctx);
...
beb_ll_error_t beb_ll_individual_secrets(const uint8_t secret[32], const beb_pubkey_t *keys, size_t keys_count, beb_secret_t **secrets_out, size_t *secrets_count_out) {
    ...
    for (size_t i = 0; i < keys_count; i++) {
        beb_ll_error_t err = beb_ll_individual_secret(secret, &keys[i], secrets[i].data);
...
beb_ll_error_t beb_ll_encrypt_with_nonce(const uint8_t secret[32], const uint8_t *data, size_t data_len, const uint8_t nonce[12], uint8_t **ciphertext_out, size_t *ciphertext_len_out) {
    ...
    /* Allocate output buffer (plaintext + 16 bytes for GCM tag + some extra for Final) */
    ...
    /* Finalize encryption (in GCM mode this should output 0 bytes, but check anyway) */
    ...
    if (EVP_CIPHER_CTX_ctrl(ctx, EVP_CTRL_GCM_GET_TAG, 16, ciphertext + len + final_len) != 1) {
        free(ciphertext);
        EVP_CIPHER_CTX_free(ctx);
        return BEB_LL_ERROR_ENCRYPT;
    }
    
    /* Total ciphertext length: encrypted data from Update + any from Final + tag */
...
beb_ll_error_t beb_ll_try_decrypt_aes_gcm_256(const uint8_t *ciphertext, size_t ciphertext_len, const uint8_t secret[32], const uint8_t nonce[12], uint8_t **plaintext_out, size_t *plaintext_len_out) {
    ...
    /* Allocate output buffer (encrypted_len + block size for potential Final output) */
    size_t plaintext_buf_len = encrypted_len + EVP_CIPHER_block_size(EVP_aes_256_gcm());
    ...
    if (EVP_DecryptUpdate(ctx, plaintext, &len, ciphertext, (int)encrypted_len) != 1) {
        free(plaintext);
        EVP_CIPHER_CTX_free(ctx);
        return BEB_LL_ERROR_DECRYPT;
    }
