...
beb_ll_error_t beb_ll_parse_derivation_paths(const uint8_t *bytes,
    ...
    if (count != 0) {
        for (uint8_t i = 0; i < count; i++) {
            ...
            err = beb_ll_check_offset_lookahead(offset, bytes, bytes_len,
                                                child_count * 4);
            if (err != BEB_LL_ERROR_OK) {
                goto error;
            }
...
beb_ll_error_t beb_ll_decode_version(const uint8_t *bytes, size_t bytes_len,
    ...
    return beb_ll_parse_version(&bytes[offset], bytes_len - offset,
...
beb_ll_error_t beb_ll_decode_derivation_paths(const uint8_t *bytes,
    ...
    size_t version_offset;
    uint8_t version;
    err = beb_ll_parse_version(&bytes[offset], bytes_len - offset,
    ...
    size_t paths_offset;
    return beb_ll_parse_derivation_paths(&bytes[offset], bytes_len - offset,
...
beb_ll_error_t beb_ll_decode_v1(const uint8_t *bytes, size_t bytes_len,
    ...
    size_t deriv_offset;
    err = beb_ll_parse_derivation_paths(&bytes[offset], bytes_len - offset,
    ...
    err = beb_ll_parse_encryption(&bytes[offset], bytes_len - offset,
                                  &encryption_offset,
                                  &result_out->encryption_type);
    if (err != BEB_LL_ERROR_OK) {
        beb_ll_derivation_paths_free(result_out->paths,
                                     result_out->paths_count);
        beb_ll_secrets_free(result_out->individual_secrets,
    ...
    err = beb_ll_parse_encrypted_payload(
