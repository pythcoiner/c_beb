...
static int test_content_metadata(void) {
    ...
    beb_ll_error_t err = beb_ll_parse_content_metadata(none_bytes, sizeof(none_bytes), &offset, &content);
    if (err != BEB_LL_ERROR_OK || content.type != BEB_CONTENT_NONE || offset != 1) {
    ...
    err = beb_ll_parse_content_metadata(bip380_bytes, sizeof(bip380_bytes), &offset, &content);
    if (err != BEB_LL_ERROR_OK || content.type != BEB_CONTENT_BIP380 || offset != 3) {
    ...
    err = beb_ll_parse_content_metadata(prop_bytes, sizeof(prop_bytes), &offset, &content);
    if (err != BEB_LL_ERROR_OK || content.type != BEB_CONTENT_PROPRIETARY || offset != 4) {
...
static int test_varint(void) {
    ...
    beb_ll_error_t err = beb_ll_varint_encode(0x42, buffer, sizeof(buffer), &written);
